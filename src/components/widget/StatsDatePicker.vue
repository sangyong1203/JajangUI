    ...
    <div class="stats-date-picker" :style="{ width: props.width ?? '' }">
        ...
        <template v-if="dateType==='date' || dateType==='month'|| dateType==='year' ">
            <el-date-picker
                ...
                :placeholder="startPlaceholder ??  L.시작날짜??'시작 날짜'"
                :style="{ flex: 1 }"
                :disabled="disabled"
            />
            <span > ~ </span>
            <el-date-picker
                ...
                :type="dateType"
                :value-format="valueFormat"
                :placeholder="endPlaceholder ?? L.종료날짜??'종료 날짜'"
                :style="{ flex: 1 }"
                :disabled="disabled"
            />
        </template>
        <template v-if="dateType==='week' || dateType==='daily'">
            <el-date-picker
                ...
                type="date"
                :value-format="valueFormat"
                :placeholder="startPlaceholder ?? L.주간선택??'주간 선택'"
                :style="{ flex: 1 }"
                :disabled="disabled"
            />
            <span > ~ </span>
...
const props = defineProps<StatsDatePickerProps>()
