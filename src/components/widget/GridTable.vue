...
const setTableData = (data:any) => {
    data.forEach( (item:any) => {
        ...
        if(item.createDate) item.createDate = item.createDate ? dateFormat(item.createDate,'YYYY-MM-DD HH:mm:ss') : ''
        if(item.updateDate) item.updateDate = item.updateDate ? dateFormat(item.updateDate,'YYYY-MM-DD HH:mm:ss') : ''
        if(item.stopDate) item.stopDate = item.stopDate ? dateFormat(item.stopDate,'YYYY-MM-DD HH:mm:ss') : ''
        if(item.completeDate) item.completeDate = item.completeDate ? dateFormat(item.completeDate,'YYYY-MM-DD HH:mm:ss') : ''
        if(item.installDate) item.installDate = item.installDate ? dateFormat(item.installDate,'YYYY-MM-DD HH:mm:ss') : ''
        if(item.releaseDate) item.releaseDate = item.releaseDate ? dateFormat(item.releaseDate,'YYYY-MM-DD HH:mm:ss') : ''
    })
    grid.value?.clear()
