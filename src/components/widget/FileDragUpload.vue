<template>
    <div style="display: flex; margin-bottom: 12px">
        ...
        <el-upload :show-file-list="false" :limit="1" :on-change="findFile" :auto-upload="false" :disabled="disabled">
            <el-button type="primary">{{ buttonName?? L.찾아보기 ?? '찾아보기' }}</el-button>
        </el-upload>
    </div>
    <el-upload
        class="upload-demo"
    ...
    >
        ...
        <div class="el-upload__text">{{ description ?? L.여기에업로드할파일을끌어서놓거나찾아보기를클릭하세요?? '여기에 업로드할 파일을 끌어서 놓거나 [찾아보기]를 클릭하세요' }}</div>
        <template #tip>
...
const onSizeOver = (file: any) => {
    Notification.warning(`${L.첨부할수있는최대파일크기를초과하였습니다최대파일크기??'첨부할 수 있는 최대 파일크기를 초과하였습니다. 최대 파일크기'} ${size.value} KB`)
}
</script>
