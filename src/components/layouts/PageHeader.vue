<template>
    <el-page-header class="page-header" :class="hideIcon ? 'hide-icon': nav.length > 2 ? '' : 'hide-icon'" >
        <template #icon>
            <el-icon @click="backTo()"><ArrowLeft /></el-icon>
        </template>
        <el-button v-show="isLoading" style="position:absolute; color: black; left: 0; top: 14px; font-size: 24px; margin-left: -10px;" text loading ></el-button>
        <template #title>
